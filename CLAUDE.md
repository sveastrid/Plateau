...
| [`bugFixesStairsGame.md`](docs/bugFixesStairsGame.md) | **Applied**, in two passes. Six changes to Stairs: out-of-turn play (the turn state moves onto the seat), the mirrored flat labels, re-laying a placed tile, why a pawn would not move, the height number moving back onto the step prefab, and lifting the tiles a player owes. Its status block records where the code came out differently from the plan. |
